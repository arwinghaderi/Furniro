...
const addingProductsTemplate = (
  products,
  productsStructure = 'row',
  productsWrapper
) => {
  document.body.classList.contains('shop-page')
...
const addingProductsTemplateRow = (product) => {
  ...
  discountPrice = `</span><span class="product-discountPrice">Rp ${product.price.toLocaleString(
    'en'
  )}</span>`

  element.innerHTML = `<div class="product-box" id="discount"><div class="img-box-overlay"><img class="product__img product__img-col" src="${fullImagePath}" onerror="this.onerror=null;this.src='https://via.placeholder.com/200?text=Furniture+Store';" alt="${
    product.name
  }" loading="lazy"><div class="product-overlay">
     <div class="box-add-btn-cart"><a href="../../Pages/product.html?slug=${
       product.slug
     }" class="box-add-btn-cart__text">Details Product</a></div>
     <div class="product-overlay__options">
     <div class="option-overlay">
     <i class="fas fa-share-alt fa-flip-vertical icon" onclick="copyToClipboard('${
       product.slug
     }')"></i>
    ...
    product._id
     ...
     <div class="option-overlay">
     <svg id="like" onclick="toggleLike(this, '${product._id}', ${
    product.isFavorite
  })" class="icon heart-icon ${
    product.isFavorite ? 'heart-icon--active' : ''
  }" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"> <path stroke-linecap="round" stroke-linejoin="round" d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41 0.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"/> </svg>
     <span class="option-overlay__text">Like</span></div></div></div><div class="lable-product-box"> ${
       product.discountPercent ? discountTemplate : ''
     } ${
    product.isNewProduct ? newTemplate : ''
  }</div></div><div class="product-Introductions"><h3 class="product-name">  ${product.name.slice(
    0,
    10
  )}...  </h3><p class="product-Introduction"> ${
    product.title
  }   </p><div class="product-price-box">
    <span class="product-price"> Rp  ${
      product.discountPercent
        ? product.priceAfterDiscount.toLocaleString('en')
        : product.price.toLocaleString('en')
    }   ${product.discountPercent ? discountPrice : ''}
    </div> </div></div>`
...
const addingProductsTemplateCol = (product) => {
  ...
  element.innerHTML = `<div class="product-box product-box-col"  id="discount"><div   
    ...
    product.isNewProduct ? newTemplate : ''
  }</div></div><div class="product-Introductions product-Introductions-col "><h3 class="product-name product-name-col ">  ${product.name.slice(
    0,
    8
  )}...  </h3><span class="product-Introduction product-Introduction-col">  ${product.title.slice(
    0,
    10
  )}... </span><div class="product-price-box"> 
    <span class="product-price"> Rp  ${
      product.discountPercent
        ? product.priceAfterDiscount.toLocaleString('en')
        : product.price.toLocaleString('en')
...
const toggleLike = async (element, productId, isFavorite) => {
  ...
  try {
    ...
    const response = await fetch(
      `https://furniro-6x7f.onrender.com/product/favorites/${productId}`,
      {
        method: method,
        headers: {
    ...
    if (response.ok) {
      ...
      element.setAttribute(
        'onclick',
        `toggleLike(this, '${productId}', ${isFavorite})`
      )
