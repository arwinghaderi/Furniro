...
const fetchLogoutUser = async () => {
  ...
  const response = await fetch(
    'https://furniro-6x7f.onrender.com/auth/log-out',
    {
      method: 'POST',
      headers: {
...
const fetchRefreshToken = async () => {
  ...
  try {
    let response = await fetch(
      'https://furniro-6x7f.onrender.com/auth/refresh',
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
    ...
    if (response.ok) {
      ...
      return {
        AccessToken: RefreshTokenData.data.newAccessToken,
        RefreshToken: RefreshTokenData.data.newRefreshToken,
      }
    } else {
      redirectToLogin()
    }
...
const btnSubmitNewsletter = document.querySelector(
  '.items-Newsletter__subscribe'
)

const fetchNewsletter = async () => {
  const emailInoptNewsletter = document.querySelector(
    '.items-Newsletter__input-email'
  )
  ...
  try {
    const response = await fetch(
      'https://furniro-6x7f.onrender.com/newsletter/',
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
    ...
    if (!response.ok) {
      const message =
        newsletterEmailData.error.message || 'An unexpected error occurred.'
