...
} from './Features/cartQuantityDisplay.js'

const $ = document
const urlParamsSlug = getUrlParam('slug')
let informationMenuItems = document.querySelectorAll(
...
informationMenuItems.forEach((infomationMenuItem) => {
  infomationMenuItem.addEventListener('click', (event) => {
    document
      .querySelector('.product-infomation-menu__item--active')
      .classList.remove('product-infomation-menu__item--active')
...
const fetchProductsDetails = async () => {
  ...
  try {
    ...
    const response = await fetch(
      `https://furniro-6x7f.onrender.com/product/${urlParamsSlug}`,
      {
        method: 'GET',
        headers: headers,
      }
...
const createStarRating = (averageRating) => {
  const filledStars =
    `<i class="fas fa-star detailes-product-Specifications__star" ></i>`.repeat(
      Math.ceil(averageRating)
    )
  const emptyStars =
    `<i class="far fa-star detailes-product-Specifications__star" ></i>`.repeat(
...
const createSizeButtons = (sizes) =>
  sizes
    .map(
      (size, index) => `
    <div class="detailes-product__size-btn-box ${
      index === 0 ? 'detailes-product__size-btn-box--active' : ''
    }"> 
        <button class="detailes-product__size-btn ${
          index === 0 ? 'detailes-product__size-btn--active' : ''
        }">${size}</button>
    </div>`
    )
    .join('')
...
<div style="background-color:${
        color.hexColorCode
      }; width: 3rem; height: 3rem; border-radius: 100%; cursor: pointer; transition: all 0.8s ease-in-out; ${
        index === 0 ? 'transform: scale(1.4);' : ''
      }" class="detailes-product__color-btn-box" data-color="${
        color.hexColorCode
      }"></div>`
    )
    .join('')
...
const updateCartCount = (input, increment, maxCount = 5) => {
  ...
  if (currentCount > maxCount) {
    showSwal(
      `If you want more than ten pieces, call this number 09308064108`,
      'warning',
      'ok',
      '#'
    )
...
const selectingCountProductByUser = (
  btnAddProductCount,
  btnReduceNumberProduct,
  productCountInput
) => {
...
const addingproductsDetailes = (productsDetailes) => {
  ...
  const discountHTML = productsDetailes.discountPercent
    ? `<h3 class="detailes-produc-Specifications__title section-title">${productsDetailes.title.slice(
        0,
        15
      )}..</h3>
           <h5 class="detailes-produc-Specifications__price-discount section-title">Rp ${productsDetailes.price.toLocaleString(
             'en'
           )}</h5>
           <h5 class="detailes-produc-Specifications__price section-title">Rp ${productsDetailes.priceAfterDiscount.toLocaleString(
             'en'
           )}</h5>`
    : `<h3 class="detailes-produc-Specifications__title section-title">${productsDetailes.title.slice(
        0,
        15
           ...
           <h5 class="detailes-produc-Specifications__price section-title">Rp ${productsDetailes.price.toLocaleString(
             'en'
           )}</h5>`
  ...
  wrapper.insertAdjacentHTML(
    ...
    `
        ...
        <div class="detailes-produc-Specifications__customer-review-box">
            ...
            <p class="detailes-produc-Specifications__status">${
              productsDetailes.averageRating
            } Total product score</p>  
        </div>
        <div class="detailes-product__description-box">
            <p class="detailes-product__description">${productsDetailes.description.slice(
              0,
              400
            )}...</p>
        </div>
        ...
        productsDetailes.quantity
         ...
         <div class="detailes-product-btn">
            ...
            <a href="../../Pages/Comparison.html?categoryId=${
              productsDetailes.categoryId._id || productsDetailes.categoryId
            }&productId=${productsDetailes._id}" > 
                <button class="detailes-product-btn__Compare">+Compare</button>
            </a>
            </div>
        </div>`
          : `<div class="detailes-product-btn"><span class='no-stock section-title'>This product is not available.</span></div>`
      }  
     `
  )

  document
    .querySelectorAll('.detailes-product__size-btn-box')
    .forEach((button) => {
      button.addEventListener('click', () => {
        handleActiveClass(
          document.querySelectorAll('.detailes-product__size-btn-box'),
          'detailes-product__size-btn-box--active',
          '.detailes-product__size-btn'
        )
        button.classList.add('detailes-product__size-btn-box--active')
        button
          .querySelector('.detailes-product__size-btn')
          .classList.add('detailes-product__size-btn--active')
      })
    })

  document
    .querySelectorAll('.detailes-product__color-btn-box')
    .forEach((button) => {
      button.addEventListener('click', () => {
        document
          .querySelectorAll('.detailes-product__color-btn-box')
          .forEach((btn) => (btn.style.transform = 'scale(1)'))
        button.style.transform = 'scale(1.4)'

        const selectedColor = button.getAttribute('data-color')
        const selectedImage = productsDetailes.images.find(
          (img) => img.hexColorCode === selectedColor
        )
        if (selectedImage) {
          document.querySelector(
            '.detailes-product-img-main___img'
          ).src = `https://furniro-6x7f.onrender.com${selectedImage.path}`
        }
      })
    })

  const btnAddProductCount = document.querySelector(
    '.detailes-product-input__quantity__plus'
  )
  const btnReduceNumberProduct = document.querySelector(
    '.detailes-product-input__quantity__minus'
  )
  const productCountInput = document.querySelector(
    '.detailes-product-input__quantity'
  )
...
const getProductDetails = (productId) => {
  const quantity = +document.querySelector('.detailes-product-input__quantity')
    .value
  const color = document.querySelector(
    '.detailes-product__color-btn-box[style*="scale(1.4)"]'
  ).dataset.color
  const size = document.querySelector(
    '.detailes-product__size-btn-box--active .detailes-product__size-btn'
  ).textContent
...
const addingPagePathDom = (productsDetails) => {
  ...
  setTimeout(() => {
    routeProduct.innerHTML = ` <div class="container "> <div class="route-product__wrapper "> <a href="${
      previousPaths[1] || 'https://furniroo-store.vercel.app/index.html'
    }" class="route-product__path-name">${
      extractedPart[1] || 'Direct Entry'
    }</a> <i class="fa-solid fa-angle-right fa-xs"></i> <a href="${
      previousPaths[0] || 'https://furniroo-store.vercel.app/index.html'
    }" class="route-product__path-name">${
      extractedPart[0] || 'home'
    }</a> <i class="fa-solid fa-angle-right fa-xs"></i> <div class="route-product__line-col line"></div> <span class="route-product__product-name">${productsDetails.name.slice(
      0,
      10
    )}</span> </div> </div> `
  }, 5000)
...
const addingAllProductPhotos = (product) => {
  const wrapperMainImage = document.querySelector(
    '.detailes-product-img-main__box'
  )
  const wrapperSecoundImg = document.querySelector(
    '.detailes-product-img-secound'
  )
  let images = product.images

  wrapperMainImage.insertAdjacentHTML(
    'afterbegin',
    `
  ...
  images.forEach((img, index) => {
    ...
    wrapperSecoundImg.insertAdjacentHTML(
      'beforeend',
      `
            <div class="detailes-product-img-secound__box ${
              index === 0 ? 'detailes-product-img-secound__box--active' : ''
            }">
                  <img class="detailes-product-img-secound__img" onerror="this.onerror=null;this.src='https://via.placeholder.com/200?text=Furniture+Store';" src="${imgPath}" alt="main-product" data-path="${
        img.path ? img.path : ''
      }" loading="lazy">
            </div>`
    )
...
const handleSecondaryImageClick = (wrapperSecoundImg, wrapperMainImage) => {
  wrapperSecoundImg.addEventListener('click', (event) => {
    ...
    if (target) {
      ...
      const mainImage = wrapperMainImage.querySelector(
        '.detailes-product-img-main___img'
      )
      ...
      document
        .querySelectorAll('.detailes-product-img-secound__box')
        .forEach((box) => {
...
const addingProductSpecifications = (product) => {
  const wrapperProductSpecifications = $.querySelector(
    '.Supplementary-specifications__value-box'
  )

  wrapperProductSpecifications.insertAdjacentHTML(
    'afterbegin',
...
const addingProductInformationTemplate = (product) => {
  ...
  const extractedAttributes = Object.entries(
    productInfomation.attributes
  ).slice(0, 3)

  extractedAttributes.forEach(([key, value]) => {
    productAttributesBody.insertAdjacentHTML(
      'beforeend',
      `
  ...
  productImgSection.insertAdjacentHTML(
    'beforeend',
    `
...
const addingProductTemplateToCart = async () => {
  ...
  cartProductsItems.length
    ? cartProductsItems.forEach((item) => {
        productImage = item.product.images.find(
          (image) => image.hexColorCode === item.color
        )
        imageUrl = productImage
          ? `https://furniro-6x7f.onrender.com${productImage.path}`
          : `https://furniro-6x7f.onrender.com${item.product.images[0].path}`

        keeperCartProduct.insertAdjacentHTML(
          'afterbegin',
          `<div class="products-keeper"><div class="products-keeper__box-img"><img onerror="this.onerror=null;this.src='https://via.placeholder.com/200?text=Furniture+Store';" loading="lazy" class="products-keeper__img" src="${imageUrl}"></div><div class="products-keeper-box-profile"><h6 class="products-keeper-box-profile__title section-title">${item.product.name.slice(
            0,
            10
          )}...</h6><div class="box-calculation"><span class="box-calculation__number">${
            item.quantity
          }</span>
              <span class="box-calculation__multiplication">x</span><span class="box-calculation__price">Rs ${
                item.product.discountPercent
                  ? item.product.priceAfterDiscount.toLocaleString('en')
                  : item.product.price.toLocaleString('en')
              } </span>
              </div>
              <div class="product-information-detailes">
                <div class="detailes-product__size-btn-box detailes-product__size-btn-box--active ">
                    <span class="detailes-product__size-btn detailes-product__size-btn--active">${
                      item.size
                    }</span>
                  </div>
                  <div class="box-shadow" style="background-color:${
                    item.color
                  }; width: 3rem; height: 3rem; border-radius: 100%;"></div>
              </div>
                  </div><button onclick="removeProductByUserByUser(this,'${
                    item._id
                  }', '${token}')"class="products-keeper-product-delete-btn"><div class="box-remove-product"> <i class="fas fa-times icon-close "></i></div></button></div>`
        )

        subTotalBoxPrice.innerHTML = `Rs.${cartProductsTotal.allPrice.toLocaleString(
          'en'
        )}`
      })
    : (keeperCartProduct.innerHTML = `  
...
const deleteProductAndUpdateCart = async (element, productId, token) => {
  ...
  try {
    const response = await fetch(
      `https://furniro-6x7f.onrender.com/cart/${productId}`,
      {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
